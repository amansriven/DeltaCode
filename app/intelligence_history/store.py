...
def _pull_request_items(workspace_id: str) -> list[dict]:
    with get_connection() as conn:
        rows = conn.execute(
            """SELECT h.id, h.repository_full_name, h.pull_number, h.head_sha,
                      h.pull_updated_at, h.input_snapshot, h.data, h.model, h.input_tokens,
                      h.cached_input_tokens, h.output_tokens, h.cost_usd, h.generated_at,
                      (c.updated_at = h.generated_at) AS is_current
               FROM pull_request_ai_overview_history h
               ...
               ORDER BY h.generated_at DESC, h.id DESC LIMIT 300""",
            (workspace_id,),
        ).fetchall()
    items = []
    for row in rows:
...
def history_detail(workspace_id: str, kind: str, item_id: str) -> dict | None:
    with get_connection() as conn:
        ...
        if kind == "pull_request":
            row = conn.execute(
                """SELECT repository_full_name, pull_number, head_sha, pull_updated_at,
                          input_snapshot, data, model, input_tokens, cached_input_tokens,
                          output_tokens, cost_usd, generated_at
                   FROM pull_request_ai_overview_history
            ...
            return {
                ...
                "pull_number": row[1],
                "head_sha": row[2],
                "pull_updated_at": row[3].isoformat() if row[3] else None,
                "snapshot": row[4],
                "overview": row[5],
                "model": row[6],
                "usage": _usage(row[7], row[8], row[9], row[10]),
                "created_at": row[11].isoformat(),
